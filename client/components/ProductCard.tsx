...
export const ProductCard = ({ product, index = 0, onImageClick }: Props) => {
  ...
  const { productCard: pc } = useAppCustomization();

  const setQty = (n: number) => updateQty(product.id, Math.max(0, n), product);

  return (
    <Tilt3D max={pc?.hoverEffect === "none" ? 0 : 9} lift={pc?.hoverEffect === "lift" ? 16 : 0} className="h-full">
      <motion.article
        initial={{ opacity: 0, y: 30 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, margin: "-50px" }}
        transition={{ duration: 0.5, delay: index * 0.06 }}
        data-hover={pc?.hoverEffect ?? "lift"}
        className="product-card group h-full"
      >
        {product.badge && pc?.showBadge !== false && (
        ...
        <button
          ...
          className="relative mb-4 grid place-items-center overflow-hidden rounded-xl bg-[radial-gradient(circle,rgba(236,72,153,0.12),transparent_60%),rgba(255,255,255,0.95)] w-full cursor-zoom-in border border-primary/10"
          style={{ height: "var(--product-card-image-h, 176px)" }}
        >
          <img
            src={product.image}
            alt={product.name}
            loading="lazy"
            className={`object-contain transition-transform duration-500 drop-shadow-lg ${
              pc?.hoverEffect === "zoom" ? "group-hover:scale-125" : "group-hover:scale-110 group-hover:-translate-y-1"
            }`}
            style={{ height: "calc(var(--product-card-image-h, 176px) - 24px)" }}
          />
        </button>
        ...
        <div className="flex items-center justify-between gap-2">
          <span
            ...
            style={{ color: "var(--product-price-color, hsl(var(--primary)))" }}
          >
            ₹{product.price}
          </span>
