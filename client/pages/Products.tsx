...
const Products = () => {
  ...
  const { productCard } = useAppCustomization();
  const [search, setSearch] = useState("");
  const [active, setActive] = useState<Product | null>(null);
  const desktopCols = productCard?.columns ?? 3;
  const gridColsClass =
    desktopCols === 2 ? "sm:grid-cols-2" : desktopCols === 4 ? "sm:grid-cols-2 xl:grid-cols-4" : "sm:grid-cols-2 xl:grid-cols-3";
